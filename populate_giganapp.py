...
def populate():
    ...
    add_figure(paper=soapdvo2_paper,
    ...
    add_table(paper=soapdvo2_paper,
    ...
    add_figure(paper=smilefinder_paper,
               title="Figure 1. Description of the SmileFinder algorithm.",
               url="",
               wf_js="/static/js/smilefinder.js",
               figure_num="1",
               wf_name="fig1_wf",
...
def add_paper(doi_suffix, gigasci_url, title, authors, article_type, shortname, journal, year, volume, page, image_url, background, results, conclusions):
...
def add_figure(paper, title, url, tbn_file, wf_js, wf_name, figure_num):
    f = Figure.objects.get_or_create(paper=paper, title=title, url=url, tbn_file=tbn_file, wf_js=wf_js, wf_name=wf_name, figure_num=figure_num)[0]
    return f


def add_table(paper, title, url, wf_js, wf_name, tbn_file, table_num):
    f = Table.objects.get_or_create(paper=paper, title=title, url=url, wf_js=wf_js, wf_name=wf_name, tbn_file=tbn_file, table_num=table_num)[0]
    return f
